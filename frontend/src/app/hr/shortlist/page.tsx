...
t { useEffect, useState, useCallback } from 'react'
import { screeningAPI, jobsAPI, applicantsAPI } from '@/lib/api'
import {
  ...
  FileText, UserCheck, Clock, Search,
} 
...
export default function HRShortlistPage() {
  ...
  const [pageReady,   setPageReady]   = useState(false)   // true after initial job list loads
  const [fetchError,  setFetchError]  = useState('')       // surfaces API errors to user
  ...
  const loadResult = useCallback(async (jobId: string) => {
    ...
    try {
      ...
      // Backend returns { success, data: ScreeningResultDoc }
      ...
      // 404 = not screened yet — not an error worth alarming the user
  ...
  // ── Initial load: fetch jobs then auto-select most recently screened ───────
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      try {
        ...
        // 2. Find the most recently screened job
        let autoJobId = ''
        try {
          ...
          if (srList.length > 0) {
            ...
            // jobId can be a populated object or a plain string/ObjectId
            autoJobId = String(
        ...
        // Fall back to first job if nothing screened
        if (!autoJobId && allJobs.length > 0) autoJobId = String(allJobs[0]._id)

        if (!cancelled && autoJobId) {
          setSelectedJob(autoJobId)
          await loadResult(autoJobId)   // ← await directly, no second useEffect chain
        }
      } catch (err: any) {
  ...
  // ── When user manually changes job selector ───────────────────────────────
  ...
  // ── Flatten candidates: prefer allCandidates, fall back to shortlist ──────
  // Guard against empty array vs undefined — check length explicitly
  ...
  return (
    <div className="space-y-5 max-w-full">
      ...
      {/* Loading a specific job's result */}
      {pageReady && loading && (
      ...
      {/* Job selected but no screening run yet */}
