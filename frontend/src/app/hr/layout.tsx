...
import {
  ...
  Bot, Star, LogOut, Bell, Search, Menu, X, ChevronLeft,
} from 'lucide-react'
...
export default function HRLayout({ children }: { children: React.ReactNode }) {
  ...
  const sideW     = collapsed ? ICON_W : FULL_W
  ...
  return (
    <div className="min-h-screen bg-sky-50">
      ...
      <aside
        style={{ width: sideW }}
        className="hidden md:flex flex-col fixed top-0 left-0 h-dvh z-50
                   bg-white border-r-2 border-sky-100
                   shadow-[2px_0_24px_rgba(14,165,233,0.08)]
                   overflow-hidden transition-[width] duration-300 ease-in-out"
      >
        {/* Brand row */}
        <div className="flex items-center gap-3 px-4 py-5 border-b border-sky-100 flex-shrink-0 overflow-hidden">
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0 overflow-hidden">
              <p className="font-display font-bold text-sky-900 text-base leading-tight whitespace-nowrap">Umurava</p>
          ...
          {navItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href} title={label}
                className={`flex items-center rounded-xl transition-all duration-150 min-h-[48px]
        ...
        <div className="border-t border-sky-100 p-3 flex-shrink-0">
          <div className={`flex items-center ${collapsed ? 'flex-col gap-2 justify-center' : 'gap-3'}`}>
            ...
            <button onClick={() => { logout(); router.push('/auth/hr') }} title="Logout"
      ...
      {/* ── Mobile backdrop ──────────────────────────────────── */}
      {drawerOpen && (
        <div className="fixed inset-0 z-40 bg-sky-900/30 backdrop-blur-sm md:hidden"
          onClick={() => setDrawerOpen(false)} />
      )}

      {/* ── Mobile drawer ────────────────────────────────────── */}
      <aside className={`md:hidden fixed top-0 left-0 h-dvh z-50 w-60 flex flex-col
                         bg-white border-r-2 border-sky-100
                         shadow-[2px_0_24px_rgba(14,165,233,0.08)]
          ...
          {navItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href}
                className={`flex items-center gap-3 px-3 rounded-xl transition-all min-h-[48px]
                  ${active
                    ? 'bg-sky-100 text-sky-700 font-semibold'
                    : 'text-sky-500 hover:bg-sky-50 hover:text-sky-700 font-medium'
                  }`}
        ...
        <div className="border-t border-sky-100 p-3 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sky-900 text-sm font-semibold truncate">{user.firstName} {user.lastName}</p>
              <p className="text-sky-400 text-xs capitalize">{user.role}</p>
            </div>
            <button onClick={() => { logout(); router.push('/auth/hr') }} title="Logout"
              className="text-sky-400 hover:text-red-400 hover:bg-red-50
      ...
      {/* ── Main content ─────────────────────────────────────── */}
      <div
        className="flex flex-col min-h-screen pb-16 md:pb-0 transition-[margin] duration-300 ease-in-out"
        style={{ marginLeft: `${sideW}px` }}
      >
        {/* Topbar */}
        <header className="sticky top-0 z-30 bg-white border-b-2 border-sky-100
                           shadow-[0_2px_12px_rgba(14,165,233,0.07)]
                           flex items-center gap-3 px-4 h-14 sm:px-5 sm:h-16 flex-shrink-0">
          {/* Mobile hamburger */}
          ...
          <h1 className="font-display font-bold text-sky-900 capitalize text-base sm:text-lg flex-1 truncate">
            {pageTitle}
          </h1>
          ...
          <div className="hidden sm:flex items-center gap-2 bg-sky-50 border-2 border-sky-100
            ...
            <input placeholder="Search…"
              className="bg-transparent text-sm text-sky-800 placeholder-sky-300 outline-none w-36 md:w-44" />
          </div>
        ...
        {navItems.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link key={href} href={href}
              ...
              {active && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-6 h-0.5 rounded-full bg-sky-500" />}
            </Link>
          )
        })}
      </nav>
