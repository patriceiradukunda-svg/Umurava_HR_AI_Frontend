    ...
    <div className="space-y-5 max-w-6xl">
      ...
      <div>
        ...
        <p className="text-sky-400 text-sm mt-1">
          Configure and launch intelligent candidate evaluation
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-5 gap-5">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className={card}>
            <div className="flex items-center gap-3 px-5 py-4 border-b border-sky-100">
              ...
              {/* Job count badge */}
              {jobs.length > 0 && (
                <span className="flex-shrink-0 text-xs font-bold bg-sky-100 text-sky-600 px-2.5 py-1 rounded-full">
            ...
            <div className="p-5 space-y-3">
              ...
              {/* ── Search bar ── */}
              {jobs.length > 0 && (
                <div className="flex items-center gap-2 border-2 border-sky-200 rounded-xl px-3 h-11
                  ...
                  {jobSearch && (
                    <button onClick={() => setJobSearch('')}
                      className="text-sky-300 hover:text-sky-600 transition-colors flex-shrink-0 w-5 h-5 flex items-center justify-center">
              ...
              {/* ── Job list ── */}
              {jobs.length === 0 ? (
                <p className="text-sky-300 text-sm text-center py-6">No jobs found. Post a job first.</p>
              ) : filteredJobs.length === 0 ? (
                <div className="text-center py-6">
                  <p className="text-sky-400 text-sm font-semibold">No jobs match "{jobSearch}"</p>
                  <button onClick={() => setJobSearch('')}
                    className="text-sky-500 text-xs underline mt-1 font-medium">
                    Clear search
                  </button>
                </div>
              ) : (
        ...
        {/* ══════════════════════════════════════════════════════════
            RIGHT — Evaluation Engine panel  (WHITE + SKY BLUE)
           ══════════════════════════════════════════════════════════ */}
        <div className="lg:col-span-2">
          <div className={`${card} sticky top-20 overflow-hidden`}>

            {/* Panel header — sky blue gradient strip */}
                ...
                <div className="bg-sky-50 rounded-xl p-3 border-2 border-sky-100">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap">
                    <span className="text-sky-500 text-xs flex items-center gap-1">
                      <Users className="w-3 h-3" />{job.applicantCount || 0} candidates
                    </span>
                    <span className="text-sky-500 text-xs flex items-center gap-1">
                      <Star className="w-3 h-3" />Top {shortlistSize}
                    </span>
                    <span className="text-sky-400 text-xs flex items-center gap-1">
                      <Hash className="w-3 h-3" />{String(job._id).slice(-6).toUpperCase()}
                    </span>
                  </div>
                </div>
              )}
                  ...
                  return (
                    <div key={step.key}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs ${
                          isDone ? 'text-emerald-500' : isActive ? 'text-sky-500' : 'text-sky-300'
                        }`}>
                          {step.sub}
                        </p>
                      </div>

                      {/* Arrow link on final step */}
